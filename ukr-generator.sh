...
if [ "$ARCH" = "RISCV" ]; then
  ./src/asm_generator/ukernels_generator.py --arch riscv --op_b ${OP,,} --reorder ${_PIPELINING} ${_REORDER} ${_UNROLL}
else
  ./src/asm_generator/ukernels_generator.py --arch armv8 ${_PIPELINING} ${_REORDER} ${_UNROLL}
fi
